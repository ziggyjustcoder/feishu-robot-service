...
public class TripMcpService {
    ...
    /**
     * 工具3：根据时间和 VIN 查询用户指定日期的 T+1 行程
     *
     * @param vin  车辆 VIN 码
     * @param date 查询日期，格式 yyyy-MM-dd
     * @return 指定日期的 T+1 行程列表
     */
    @Tool(name = "queryDailyTripByDate",
            description = "根据车辆 VIN 和日期查询该日期的 T+1（离线处理完成）行程列表。" +
    ...
    @Tool(name = "queryIntelligentDriveData",
            description = "根据车辆 VIN 和日期查询该日期的智驾（辅助驾驶）统计数据。" +
